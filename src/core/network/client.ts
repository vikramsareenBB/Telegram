...
function buildJSONConfig(payload: unknown): Promise<AxiosRequestConfig> {
  return Promise.resolve({
    ...
    data: JSON.stringify(payload, replacer),
  })
}
...
async function attachFormMedia(
  ...
  if ('url' in media && media.url !== undefined) {
    ...
    const response = await axios.get(media.url, config)
...
function redactToken(error: AxiosError | Error): never {
...
class ApiClient {
  ...
  async callApi<M extends keyof Telegram>(
    ...
    if (
      ...
      debug('Call via webhook', method, payload)
      // @ts-expect-error using webhookReply is an optimization that doesn't respond with normal result
      // up to the user to deal with this
      return await answerToWebhook(response, { method, ...payload }, options)
    }
    ...
    const config: AxiosRequestConfig = includesMedia(payload)
      ? await buildFormDataConfig(
          { method, ...payload },
          options.attachmentAgent
        )
    ...
    try {
      const res = await axios(apiUrl.toString(), config)
      if (res.status >= 500) {
        const errorPayload = {
          error_code: res.status,
          description: res.statusText,
